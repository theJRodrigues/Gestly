...
import { ValidateUniqueCustomerService } from "@shared/services";
...
export class CreateCustomerController implements ICreateCustomerController {
  ...
  async create(
    ...
    try {
      const validateNotUniqueness = new ValidateUniqueCustomerService(
        this.repository
      );

      const isNotUnique = await validateNotUniqueness.notUnique(customer);
